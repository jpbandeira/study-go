...
func BasicConstructsAndElementaryDataTypes() {
	...
	// TIMES AND DATES
	t := time.Now().UTC()
	fmt.Println("Time now: ", t.Format(time.RFC850))
	fmt.Printf("Dia: %02d Mes: %02d Ano: %4d\n", t.Day(), t.Month(), t.Year())
	...
	int1 := 10
	fmt.Printf("Valor preenchido em int1: %d\n", int1)
	var int2 *int
	int2 = &int1
	fmt.Printf("Valor para posição de memoria onde int1 se encontra atribuido a int2: %d\n", int2)
	fmt.Printf("Tratando int2 como ponteiro para pegar valor referente a variavel apontada: %d", *int2)

	// const i = 5
	// ptr1 := &i //error: cannot take the address of i
	// ptr2 := &10 //error: cannot take the address of 10

	// can't assigne to a pointer the value nil
}
